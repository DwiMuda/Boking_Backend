<template>
  <div>
    ...
    <div v-else-if="service" class="card" style="max-width:600px">
      <h2>{{ service.nama }}</h2>
      <p style="margin:1rem 0;color:#666">{{ service.deskripsi }}</p>
      <div class="price">Rp {{ formatHarga(service.harga) }}</div>
      <div class="duration" style="margin:0.5rem 0 1.5rem">Durasi: {{ service.durasi_menit }} menit</div>
      <routerLink :to="`/booking/${service.id}`" class="btn btn-primary">
        Booking Sekarang
      </routerLink>
      <routerLink to="/services" class="btn btn-outline" style="margin-left:0.5rem">
        Kembali
      </routerLink>
    </div>
  </div>
